...
- Update/Add Feature Labels: Add or update labels in the `featureFlags` array to define status labels such as `Preview`,
  `New`, or `Beta`.
- Use Feature Flags in Your Code: Conditionally show/hide UI elements or run logic based on the feature flag, and change UI
  behaviour depending on feature labels.

## Conditionally Enabling/Disabling Features

You can use feature flags to conditionally disable or enable features and control whether certain UI elements are shown in the
application.
...
In addition to the `enabled` and `disabledFeatures` based approach above, identity-apps also supports a `featureFlags` array
inside a feature config. This is commonly used to attach a UI status label such as `NEW`, `PREVIEW`, `BETA`, or
`COMING_SOON` to a feature or sub-feature.

Unlike `disabledFeatures`, these labels do not automatically hide or disable UI elements. Instead, your component reads the
configured label and decides how to display it.

### Step-by-Step Guide

1. Identify the High-Level Feature

First, identify the parent feature that owns the UI element. For example, assume we want to show a status label for a new
"Export Users" button in the users page. In this case, the high level feature is `users`.
...
In the above example, if the configured flag value is `NEW`, the button text becomes:

`Export Users [New]`

4. Use `FeatureFlagLabel` for Standard UI Labels

If you want to render the status label as a reusable UI element instead of directly modifying the text, you can use the
`FeatureFlagLabel` component. This component reads the flag and renders it as a `chip` or a `ribbon`.
...
This is the recommended approach when you want a consistent visual representation for labels such as `Preview`, `New`, or
`Beta` across the application.
...
guide, you can define feature flags, manage them in your configuration, use them to conditionally enable or disable features,
and use feature labels to control UI behaviour and appearance in your application dynamically.
